...
### 时间复杂度O(nlongn)/空间复杂度O(n)/稳定  

### 思想

自底向上
想数据不断二分，最后分割成不可分割的单元，再将单元两两进行合并。
...
将数据进行heapify，形成一个堆结构。
每次从堆顶取出一个元素，进行shiftdown操作，直到将元素取完
由于shfit不是相邻元素进行比较，故有可能导致相同的元素后者到前者之前，故不稳定
...
function heapSort(array){
  ...
  for(let i = last ; i >= 0  ; i --){
    __shiftDown(array,i,array.length -1)
  }

  for(let i = 0; i < array.length ; i ++){
    [array[i],array[array.length - 1 - i]] = [array[array.length - 1 - i],array[i]]
    __shiftDown(array,0,array.length-1-i)
  }
}
